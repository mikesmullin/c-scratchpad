...
DecoratorNode* CreateDecoratorNode(BTNode* child) {
  DecoratorNode* node = malloc(sizeof(DecoratorNode));
  node->child = child;
  node->base.tick = (BTStatus(*)(BTNode*))DecoratorTick;
  return node;
}
...
BTStatus WaitTick(WaitNode* node) {
  ...
  if (difftime(currentTime, node->startTime) >= node->waitTime) {
    return BT_SUCCESS;
  }
...
SelectorNode* CreateSelectorNode(BTNode** children, int count) {
  ...
  node->childCount = count;
  node->currentChild = 0;
  node->base.tick = (BTStatus(*)(BTNode*))SelectorTick;
  return node;
}
...
BTStatus RandomTick(RandomNode* node) {
  ...
  // Pick a random child
  int randomIndex = rand() % node->childCount;
...
BTStatus PriorityTick(PriorityNode* node) {
  ...
  if (node->currentTime - node->lastExecutionTime < node->period) {
    return BT_RUNNING;  // Not enough time has passed, stay in running
  }

  node->lastExecutionTime = node->currentTime;  // Update last execution time
...
PriorityNode* CreatePriorityNode(BTNode** children, int count, float period) {
  ...
  node->currentTime = 0.0f;  // Initialize current time
