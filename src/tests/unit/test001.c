...
// Selector Node: Runs child behaviors in sequence until one succeeds
typedef struct {
  BTNode base;
  BTNode** children;
  int childCount;
...
BTStatus SelectorTick(SelectorNode* node) {
  for (int i = 0; i < node->childCount; ++i) {
    ...
    BTStatus status = child->tick(child);
    if (status == BT_SUCCESS || status == BT_RUNNING) {
      return status;  // Return success or running if any child succeeds
    }
  }
  return BT_FAILURE;  // Fail if all children fail
}

// Function to create a selector node
SelectorNode* CreateSelectorNode(BTNode** children, int count) {
  SelectorNode* node = (SelectorNode*)malloc(sizeof(SelectorNode));
...
static void Test001__Test() {
  LOG_DEBUGF("Test001 BehaviorTree System Core");
